            ...
            <form action="#" method="POST" class="modal-body h-fit flex flex-col gap-2">
                <div class="modal-header mb-4">
                    <div class="flex gap-2 justify-center items-center text-green-600 text-xl">
                        <i class="bi bi-check-circle-fill"></i>
                        <h1 class="modal-title fs-5 font-bold" id="approveModalLabel">Approve <?= $request['requester'] ?? 'User' ?>'s Request</h1>
                    </div>
                    <button type="button" class="btn-close hover:text-green-500" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="flex flex-col gap-2">
                    <h3 class="text-lg">
                        Are you sure you want to approve <span class="font-bold text-[#434F72]"><?php echo $request['requester'] ?? 'requester' ?></span>'s request?
                    </h3>
                    <p class="text-[#434F72] mb-2"><?php echo $request['request'] ?? 'Request Description' ?></p>
                </div>
